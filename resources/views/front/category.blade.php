                        ...
                        <div
                            ...
                            <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                                <div
                                    ...
                                    <p class="text-xs leading-[18px] font-bold">{{ $category->name }}</p>
                                </div>
                                <img src="{{ asset('storage/' . $news->thumbnail) }}" alt="thumbnail photo"
                                    class="w-full h-full object-cover" />
                            </div>
                            <div class="flex flex-col gap-[6px]">
                                <h3 class="text-lg leading-[27px] font-bold">{{ str()->limit($news->name, 50) }}</h3>
                                <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $news->created_at->format('d M, Y') }}
                                </p>
                            </div>
                        </div>
                    </a>
